...
if (role(['permissions' => ['groups' => 'embed_group']])) {
    ...
    if (!empty($group_id)) {
        ...
        if ($no_error) {
            ...
            $embed_code = '<iframe width="100%" height="650px" allow="camera;microphone" ';
            $embed_code .= 'src="'.$group_url.'?display_chat_alone=true" frameborder=0 allowfullscreen></iframe>';

            $embed_code = htmlspecialchars($embed_code, ENT_QUOTES, 'UTF-8');

            $form['fields']->embed_code_dcm = [
                "title" => Registry::load('strings')->embed_code_dcm, "tag" => 'textarea', "class" => 'field',
                "value" => $embed_code,
            ];
            $form['fields']->embed_code_dcm['attributes']['rows'] = 8;
            $form['fields']->embed_code_dcm['attributes']['class'] = 'copy_to_clipboard';
        }
    }
