...
if (role(['permissions' => ['site_users' => ['create_user', 'edit_users'], 'profile' => 'edit_profile'], 'condition' => 'OR'])) {
    ...
    if (role(['permissions' => ['site_users' => 'pin_users']])) {
    ...
    $form['fields']->disable_non_friends_messages = [
        "title" => Registry::load('strings')->disable_non_friends_messages, "tag" => 'select', "class" => 'field'
    ];
    $form['fields']->disable_non_friends_messages['options'] = [
        "yes" => Registry::load('strings')->yes, "no" => Registry::load('strings')->no,
    ];

    if (empty($user_id) || role(['permissions' => ['profile' => 'disable_private_messages']])) {
        $form['fields']->disable_private_messages = [
            "title" => Registry::load('strings')->disable_private_chats, "tag" => 'select', "class" => 'field'
        ];
    ...
    if (empty($user_id) || role(['permissions' => ['profile' => 'disable_private_video_chat']])) {
        $form['fields']->disable_private_video_chat = [
            "title" => Registry::load('strings')->disable_private_video_chats, "tag" => 'select', "class" => 'field'
        ];
        $form['fields']->disable_private_video_chat['options'] = [
            "yes" => Registry::load('strings')->yes, "no" => Registry::load('strings')->no,
        ];
    }
    ...
    if (!empty($user_id)) {
        $disable_private_messages = $disable_private_video_chat = $disable_non_friends_messages = 'no';
        $pin_user = 'no';
        ...
        $columns = [
            ...
            'site_users.profile_cover_pic', 'site_users_settings.disable_private_video_chat',
            'site_users_settings.disable_non_friends_messages', 'site_users.pin_user'
        ];
        ...
        if ((int)$user['disable_private_video_chat'] === 1) {
            $disable_private_video_chat = 'yes';
        }

        if ((int)$user['disable_non_friends_messages'] === 1) {
            $disable_non_friends_messages = 'yes';
        }

        if (!empty($user['pin_user'])) {
            $pin_user = 'yes';
        }
        ...
        $form['fields']->disable_private_video_chat['value'] = $disable_private_video_chat;
        $form['fields']->disable_non_friends_messages['value'] = $disable_non_friends_messages;
        ...
        if (isset($form['fields']->pin_user)) {
            $form['fields']->pin_user['value'] = $pin_user;
            $form['fields']->user_order['value'] = $user['pin_user'];
