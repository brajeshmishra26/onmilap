...
if (role(['permissions' => ['site_users' => 'manage_user_access_logs']])) {
    ...
    if (isset($data["user_id"])) {
        ...
        if ((int)$data["user_id"] === (int)Registry::load('current_user')->id) {
            $own_profile = true;
        }
        ...
        $where["ORDER"] = [
            "site_users_device_logs.created_on" => "DESC",
            "site_users_device_logs.access_log_id" => "DESC",
            "site_users_device_logs.user_id"
        ];

        $device_logs = DB::connect()->select('site_users_device_logs', $join, $columns, $where);
        ...
        if (!$own_profile && Registry::load('current_user')->site_role_attribute !== 'administrators') {
            if (isset($device_logs[0])) {
                if ((int)$device_logs[0]['role_hierarchy'] >= (int)Registry::load('current_user')->role_hierarchy) {
                    return;
                }
            }
        }
