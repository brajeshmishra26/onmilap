...
if (!empty($group_id)) {
    ...
    $columns = [
        ...
        'group_members.group_member_id', 'groups.group_picture', 'groups.group_cover_pic'
    ];
    ...
    if (isset($group[0])) {
            ...
            if (!isset($group[0]['group_member_id']) || empty($group[0]['group_member_id'])) {
                ...
                if (isset($_COOKIE['current_group_secret_code']) && !empty($_COOKIE['current_group_secret_code'])) {
                    if ($_COOKIE['current_group_secret_code'] === $group[0]['secret_code']) {
                        ...
                        if (!empty($user_id) && !empty($group[0]['password'])) {
                            Registry::load('config')->join_group_conversation = $load_group_conversation;
                            Registry::load('config')->join_group_secret_code = $group[0]['secret_code'];
                        } else {
                            Registry::load('config')->load_group_conversation = $load_group_conversation;
                        }
                    }
                } else if (empty($group[0]['secret_group']) && !empty($user_id) && !empty($group[0]['password'])) {
                    Registry::load('config')->join_group_conversation = $load_group_conversation;
                    Registry::load('config')->join_group_secret_code = '';
                }
            }
        }
        ...
        if (!$skip_meta_tags) {
            ...
            if (empty($group[0]['slug'])) {
                $group[0]['slug'] = Registry::load('config')->group_url_path.'/'.$group[0]['group_id'];
            }
            ...
            $group_cover_image = get_img_url(['from' => 'groups/cover_pics', 'image' => $group[0]['group_cover_pic']]);

            if (basename($group_cover_image) !== 'default.png') {
                $meta_tags['social_share_image'] = $group_cover_image;
            }
        }
    }
}
