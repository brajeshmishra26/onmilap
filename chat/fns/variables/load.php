...
$settings = extract_json(['file' => 'assets/cache/settings.cache', 'use_redis' => true]);
...
$site_role_attributes_data = extract_json(['file' => 'assets/cache/site_role_attributes.cache', 'use_redis' => true]);
...
$apperance->main_window_style = '';
$apperance->display_chat_alone = false;

if ($settings->color_scheme == 'dark_mode') {
    $apperance->body_class = 'dark_mode';
}

if (isset($strings->language_font) && !empty($strings->language_font)) {
    $settings->default_font = $strings->language_font;
}
...
if ($settings->disable_right_click === 'yes') {
    $apperance->body_class = $apperance->body_class.' right_click_disabled';
}

$permissions = extract_json(['file' => 'assets/cache/site_roles.cache', 'extract' => $current_user->site_role, 'use_redis' => true]);
$group_role_attributes_data = extract_json(['file' => 'assets/cache/group_role_attributes.cache']);

if (!isset($permissions['role_hierarchy'])) {
    $permissions['role_hierarchy'] = 1;
}

if (!isset($permissions['site_role_attribute'])) {
    $permissions['site_role_attribute'] = 'custom_site_role';
}
$current_user->site_role_attribute = $permissions['site_role_attribute'];
$current_user->role_hierarchy = $permissions['role_hierarchy'];
