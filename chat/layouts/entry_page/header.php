...
<head>
    ...
    <link href="<?php echo Registry::load('config')->site_url.'assets/css/combined_css_entry_page_libraries_'.$css_file_cache_timestamp.'.css'; ?>" rel="stylesheet">
    <link href="<?php echo Registry::load('config')->site_url.'assets/css/combined_css_entry_page_'.$css_file_cache_timestamp.'.css'; ?>" rel="stylesheet">
    ...
    <script src="<?php echo Registry::load('config')->site_url.'assets/js/combined_js_entry_page_libraries_'.$js_file_cache_timestamp.'.js'; ?>"></script>

    <?php if (isset(Registry::load('settings')->captcha) && Registry::load('settings')->captcha === 'google_recaptcha_v2') {
        ?>
        <script src="https://www.google.com/recaptcha/api.js?hl=<?php echo Registry::load('strings')->iso_code ?>" async defer></script>
        <?php
    } else if (isset(Registry::load('settings')->captcha) && Registry::load('settings')->captcha === 'hcaptcha') {
        ?>
        <script src="https://js.hcaptcha.com/1/api.js?hl=<?php echo Registry::load('strings')->iso_code ?>" async defer></script>
        <?php
    } else if (isset(Registry::load('settings')->captcha) && Registry::load('settings')->captcha === 'cloudflare_turnstile') {
        ?>
        <script src="https://challenges.cloudflare.com/turnstile/v0/api.js" async defer></script>
        <?php
    } else if (isset(Registry::load('settings')->captcha) && Registry::load('settings')->captcha === 'friendly_captcha') {
        ?>
    ...
    <?php include 'assets/headers_footers/entry_page/header.php'; ?>
