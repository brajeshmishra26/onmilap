...
require 'fns/php_jwt/autoload.php';
...
if (!empty($social_login_provider_id)) {
    ...
    $columns = [
        ...
        'social_login_providers.additional_credentials'
    ];
    ...
    if (isset($login_provider[0])) {
        ...
        $provider_config['callback'] = Registry::load('config')->site_url.Registry::load('config')->authentication_page_url_path.'/social_login/';
        ...
        if (!empty($login_provider['additional_credentials'])) {
            $additional_credentials = json_decode($login_provider['additional_credentials'], true);

            if (!empty($additional_credentials)) {
                if ($identity_provider === 'Mastodon') {
                    if (isset($additional_credentials['app_url'])) {
                        $provider_config['providers'][$identity_provider]['url'] = $additional_credentials['app_url'];
                    }
                } else if ($identity_provider === 'Keycloak') {
                    if (isset($additional_credentials['app_url'])) {
                        $provider_config['providers'][$identity_provider]['url'] = $additional_credentials['app_url'];
                    }
                    if (isset($additional_credentials['keycloak_realm'])) {
                        $provider_config['providers'][$identity_provider]['realm'] = $additional_credentials['keycloak_realm'];
                    }
                } else if ($identity_provider === 'Apple') {
                    if (isset($additional_credentials['team_id'])) {
                        $provider_config['providers'][$identity_provider]['keys']['team_id'] = $additional_credentials['team_id'];
                    }

                    if (isset($additional_credentials['key_id'])) {
                        $provider_config['providers'][$identity_provider]['keys']['key_id'] = $additional_credentials['key_id'];
                    }

                    if (isset($additional_credentials['key_content'])) {
                        $key_content = $additional_credentials['key_content'];

                        if (strlen($key_content) < 100 & file_exists($key_content) && is_file($key_content)) {
                            $provider_config['providers'][$identity_provider]['keys']['key_file'] = $key_content;
                        } else {
                            $provider_config['providers'][$identity_provider]['keys']['key_content'] = $key_content;
                        }
                    }
                }
            }
    ...
    <script>
        if (window.opener !== null && window.opener.closePopupWindow) {
            ...
            window.location = '<?php echo Registry::load('config')->site_url.Registry::load('config')->authentication_page_url_path; ?>/';
        }
    </script>
