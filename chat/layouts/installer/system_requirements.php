...
$requirements = [
    'php_version' => [
        'title' => 'PHP Version',
        'description' => 'Requires at least PHP version 8.0 or higher. We highly recommend using the latest versions of PHP.',
        'required' => true,
    ],
    'server_software' => [
...
} else if (file_exists('htaccess.backup')) {
    $htaccess_content = file_get_contents('.htaccess');
    $searchLine = 'RewriteRule ^ - [E=SUBFOLDER:%1]';

    if (strpos($htaccess_content, $searchLine) === false) {
        rename('htaccess.backup', '.htaccess');
    }
}
